...
{	 
	...
	TIM_ITConfig(TIM4,TIM_IT_Update|TIM_IT_CC1,ENABLE);//允许更新中断 ,允许CC4IE捕获中断	
...
{ 
 if((TIM4CH1_CAPTURE_STA&0X80)==0)//还未成功捕获	
	...
	if (TIM_GetITStatus(TIM4, TIM_IT_CC1) != RESET)//捕获4发生捕获事件
		{	
			...
			}else  								//还未开始,第一次捕获上升沿
			{
		   	...
		   	TIM_OC1PolarityConfig(TIM4,TIM_ICPolarity_Falling);		//CC4P=1 设置为下降沿捕获
			}		    
		}
	}
